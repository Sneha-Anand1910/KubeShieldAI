...
# Kubernetes control-plane / system pods (kube-proxy, CNI plugins like
# Calico/Flannel/Cilium, etc.) legitimately run as root/privileged.
# Same exclusion already used in analyzers/rbac and analyzers/network —
# without it these show up as false positives on every scan.
SAFE_NAMESPACES = {
    "kube-system",
    "kube-public",
    "kube-node-lease",
