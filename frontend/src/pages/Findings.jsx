...
const Badge = ({ severity }) => (
  <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 10, fontWeight: 600, fontFamily: 'var(--font-mono)', letterSpacing: '0.08em', textTransform: 'uppercase', color: SEV_COLORS[severity], background: `${SEV_COLORS[severity]}20`, border: `1px solid ${SEV_COLORS[severity]}40` }}>
    {severity}
  </span>
)
  ...
  <div style={{ width: 360, background: 'var(--bg-surface)', borderLeft: '1px solid var(--border)', display: 'flex', flexDirection: 'column', animation: 'fade-up 0.2s ease', overflow: 'auto' }}>
    <div style={{ padding: '20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
      <div>
        ...
        <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', marginTop: 8, lineHeight: 1.4 }}>{f.Issue}</div>
      </div>
    ...
    <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        ...
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--cyan)' }}>{f.resource}</div>
      </div>
      <div>
        <div style={{ fontSize: 10, color: 'var(--text-muted)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 8 }}>Detail</div>
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6 }}>{f.Detail}</div>
      </div>
      <div>
        <div style={{ fontSize: 10, color: 'var(--text-muted)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 8 }}>Recommendation</div>
      ...
      <div>
        <div style={{ fontSize: 10, color: 'var(--text-muted)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 8 }}>Check module</div>
        <span style={{ padding: '3px 10px', borderRadius: 4, fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--cyan)', background: 'var(--cyan-dim)', border: '1px solid var(--border)' }}>{f.check}</span>
      </div>
    </div>
  </div>
)
  ...
  if (!scanResult) {
    return (
      <div style={{ padding: '32px 40px', animation: 'fade-up 0.3s ease' }}>
        ...
        <h1 style={{ fontSize: 28, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 24 }}>Findings</h1>
        <div style={{ padding: '48px', textAlign: 'center', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)' }}>
          ...
          <button onClick={() => onNav('ingest')} style={{ padding: '10px 24px', background: 'var(--cyan)', color: '#080C14', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font-ui)' }}>
            Go to Ingest →
          </button>
        </div>
      </div>
    ...
    <div style={{ display: 'flex', height: '100%', animation: 'fade-up 0.3s ease' }}>
      <div style={{ flex: 1, overflow: 'auto' }}>
        ...
        <div style={{ padding: '0 40px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr 100px 80px', gap: 12, padding: '10px 20px', borderBottom: '1px solid var(--border)' }}>
            {['Severity', 'Finding', 'Module', 'Resource'].map(h => (
              <div key={h} style={{ fontSize: 10, color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500 }}>{h}</div>
            ))}
          </div>
          {filtered.map((f, i) => (
            <div key={i} onClick={() => setSelected(selected === f ? null : f)} style={{ display: 'grid', gridTemplateColumns: '90px 1fr 100px 80px', padding: '14px 20px', borderBottom: '1px solid var(--border)', alignItems: 'center', gap: 12, cursor: 'pointer', background: selected === f ? 'var(--cyan-glow)' : 'transparent', borderLeft: selected === f ? '2px solid var(--cyan)' : '2px solid transparent', transition: 'background 0.15s' }}
            ...
            >
              <Badge severity={f.Severity} />
              <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)' }}>{f.Issue}</div>
              <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--cyan)', background: 'var(--cyan-dim)', border: '1px solid var(--border)' }}>{f.check}</span>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.resource}</div>
            </div>
          ))}
